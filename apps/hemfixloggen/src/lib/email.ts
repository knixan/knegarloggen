...
const from = process.env.RESEND_FROM ?? "noreply@hemfixloggen.se";

export async function sendVerificationEmail(email: string, url: string) {
  const { error } = await resend.emails.send({
    from,
    to: email,
    subject: "Bekräfta din e-postadress – Hemfixloggen",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2 style="color: #2563EB;">Välkommen till Hemfixloggen!</h2>
...
export async function sendPasswordResetEmail(email: string, url: string) {
  const { error } = await resend.emails.send({
    from,
    to: email,
    subject: "Återställ ditt lösenord – Hemfixloggen",
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
