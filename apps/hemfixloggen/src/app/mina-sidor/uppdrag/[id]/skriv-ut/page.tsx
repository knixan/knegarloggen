...
import { reserverFakturanummer } from "@/lib/uppdrag-actions";
...
async function SkrivUtInnehall({ params }: { params: Promise<{ id: string }> }) {
  ...
  uppdrag.fakturaNummer = fakturaResult.nummer;
    ...
    <div className="max-w-2xl mx-auto p-8 print:p-0 font-sans text-sm">
      ...
      {uppdrag.customer && (
        <div className="mb-8">
          ...
          {uppdrag.customer.personnummer && (
            <p className="text-xs">Personnummer: {uppdrag.customer.personnummer}</p>
          )}
          {uppdrag.rutAvdrag && (
            <p className="text-xs text-gray-500">RUT-avdrag tillämpas på arbetskostnaden</p>
          )}
        </div>
      )}
      ...
      <div className="border-t pt-4 space-y-1 text-sm">
        ...
        {totals.rutAvdrag > 0 && (
          <div className="flex justify-between text-red-600">
            <span>RUT-avdrag 50% (av arbetskostnad inkl. moms)*</span>
      ...
      {uppdrag.rutAvdrag && (
        <p className="mt-2 text-xs text-gray-500">
          * RUT-avdrag: Avser 50% av arbetskostnad inkl. moms. Utföraren
